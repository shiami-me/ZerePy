...
from src.tools.privy.silo_tools import SiloWithdrawToolPrivy
...
logger = logging.getLogger("test_privy_silo_withdraw")
...
def test_silo_withdraw(token_0, token_1, amount, sender, collateral_type, receiver=None, id=None):
    """Test Privy Silo withdraw functionality"""
    try:
        # Create mock agent and LLM
        agent = MockAgent()
        llm = MockLLM()
        
        # Create Silo withdraw tool
        silo_withdraw_tool = SiloWithdrawToolPrivy(agent, llm)
        
        # Execute withdraw
        result = silo_withdraw_tool._run(
            token_0=token_0,
            token_1=token_1,
            amount=float(amount),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Privy Silo withdraw functionality")
    parser.add_argument("--token0", "-t0", required=True, help="Symbol of the token to withdraw (e.g., USDC)")
    parser.add_argument("--token1", "-t1", required=True, help="Symbol of the other token in the pair (e.g., S)")
    parser.add_argument("--amount", "-a", required=True, help="Amount to withdraw")
