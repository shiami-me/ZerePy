...
from src.actions import sonic_actions
from src.tools.privy.silo_tools import SiloDepositToolPrivy
...
logger = logging.getLogger("test_privy_silo_deposit")
...
def test_silo_deposit(token_0, token_1, amount, sender, collateral_type, id=None):
    """Test Privy Silo deposit functionality"""
    try:
        # Create mock agent and LLM
        agent = MockAgent()
        llm = MockLLM()
        
        # Create Silo deposit tool
        silo_deposit_tool = SiloDepositToolPrivy(agent, llm)
        
        # Execute deposit
        result = silo_deposit_tool._run(
            token_0=token_0,
            token_1=token_1,
            amount=float(amount),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Privy Silo deposit functionality")
    parser.add_argument("--token0", "-t0", required=True, help="Symbol of the token to deposit (e.g., USDC)")
    parser.add_argument("--token1", "-t1", required=True, help="Symbol of the other token in the pair (e.g., S)")
    parser.add_argument("--amount", "-a", required=True, help="Amount to deposit")
    parser.add_argument("--sender", "-s", required=True, help="Sender wallet private key")
    parser.add_argument("--collateral-type", "-c", default=0, help="Collateral type (0 for Protected, 1 for Collateral)")
